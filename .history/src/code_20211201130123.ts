    ...
    for (var i = 0; i < pages.length; i++) {
      ...
      if (targetLayers.length > 0) {
        ...
        for (var j = 0; j < targetLayers.length; j++) {
          ...
          var thisChildren = targetLayers[j]['children']
          while (true) {
            ...
            if (thisChildren != null) {
              ...
              // 判断子图层是否为文本图层
              if (thisChildren.type == 'TEXT') {
                // 若是则获取文本图层的字符信息赋值给新图层
                tableChildren.characters = thisChildren.characters
                break
              }else{
                // 不是文本图层
                thisChildren = thisChildren['children']
              }
          ...
          if (targetLayers[j].findOne(n => n.type === "TEXT") == null) {
            tableChildren.characters = targetLayers[j].name
          } else {
            //文本值
